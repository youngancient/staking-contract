...
contract StakeERC20 is ReentrancyGuard {
    ...
    error InSufficientFundsInToken();
    ...
    uint constant MAX_STAKE_VALUE = 1000000000 * 1e18;
    ...
    // it is assumed that the token is of 18 decimals
    function addTokenRewards(uint _tokens) public {
        ...
        if(_tokens > tokenContractBalance){
            revert InSufficientFundsInToken();
        }
    ...
    function _setGeneralStakingDuration(uint _days) private {
        ...
        // stakeDuration = (_days) + block.timestamp;   -> for testing in secs
    ...
    // users can stake an amount of token only during the staking window
    ...
    function _endStaking() private {
        ...
        // send token
        IERC20(tokenAddress).transfer(msg.sender, _stakedAmount);
    }
}
